# assumptions: just one nucleation class. solution: train the following size classes considering the first size class trainables is fixed
# trainables: param1, ..., param5
# paramrpc is not a trainable parameter.
...
#issue: when driving force is positive, Rs_ is -0,  b7 is -inf, vel_growth is -inf
#solution: when driving force is positive, Rs is 1, b7 is 1, vel_growth is big,
...
# opt = tf.keras.optimizers.legacy.SGD(learning_rate=100.)
# opt = tf.keras.optimizers.legacy.SGD(learning_rate=LR_SGD)
# opt = tf.keras.optimizers.legacy.Adam(learning_rate=LR_Adam)
...
def loss():
    ...
    visual(time_, TND_t, MPR_t, TVF_t, Yield_t, loss_t, Yield_interpolated, x, y, N_optimizer, param1_t, param2_t, param3_t, param4_t, param5_t, paramrpc_t, grad1_t, grad2_t, grad3_t, grad4_t, grad5_t, iteration = ii, "ADAM")
    print(f'iteration {ii} finished with loss {loss_} and reg {l2_reg.numpy() * lambda_l2}')
    print('\n')
    return loss_


# param1 = tf.Variable(1.0, trainable = True, dtype=np.float32) # 1.0
# param2 = tf.Variable(0.1133, trainable = True, dtype=np.float32) # 1.12 0.1133
# param3 = tf.Variable(1.05, trainable = True, dtype=np.float32) # 1.05
# param4 = tf.Variable(1.1, trainable = True, dtype=np.float32) # 1.1
# param5 = tf.Variable(1.07, trainable = True, dtype=np.float32) # 1.07
paramrpc = tf.Variable(0.85, trainable = False, dtype=np.float32) # 0.85

param1 = tf.Variable(tf.random.normal([], mean=1.0, stddev=0.1), trainable=True, dtype=np.float32)
initial_p2 = tf.math.log(tf.random.normal([], mean=1.0, stddev=0.01))
param2 = tf.Variable(initial_p2, trainable=True, dtype=tf.float32)
param3 = tf.Variable(tf.random.normal([], mean=1.0, stddev=0.1), trainable=True, dtype=np.float32)
param4 = tf.Variable(tf.random.normal([], mean=1.0, stddev=0.1), trainable=True, dtype=np.float32)
param5 = tf.Variable(tf.random.normal([], mean=1.0, stddev=0.1), trainable=True, dtype=np.float32)
...
for ii in range(N_optimizer):
    ...
    if ii > 0:
        ...
        tolerance = ftol * (1 + abs(current_loss))
        print(f"Convergence reached at iteration {ii}: loss_diff={loss_diff:.6f} < tolerance={tolerance:.6f}")
        if loss_diff < tolerance:
            print(f"Convergence reached at iteration {ii}: loss_diff={loss_diff:.6f} < tolerance={tolerance:.6f}")
            break
    # print("paramrpc:", paramrpc_t)
    # opt.minimize(loss, var_list=[ param1, param2, param3, param4, param5, paramrpc]) # var_list=[param1, param2, param3, param4, param5, paramrpc]

toc = time.time()
print(f'exexution time is {(toc-tic)/60.} mins')
